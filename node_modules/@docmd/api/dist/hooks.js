...
// Isolation wrapper (§2)
// ---------------------------------------------------------------------------
function safeCall(hookName, pluginName, fn, ...args) {
    try {
        return fn(...args);
    }
    catch (err) {
 ...
 */
async function autoInstallPlugin(packageName) {
    ...
    switch (pkgManager) {
        case 'pnpm':
            installCmd = `pnpm add ${versionedPackage}`;
            break;
        case 'yarn':
            installCmd = `yarn add ${versionedPackage}`;
            break;
        case 'bun':
            installCmd = `bun add ${versionedPackage}`;
            break;
        default:
            installCmd = `npm install ${versionedPackage}`;
            break;
    }
    try {
...
function registerPlugin(name, plugin, options) {
    ...
    if (typeof plugin.generateMetaTags === 'function') {
        if (hasCapabilityForHook(descriptor, 'generateMetaTags')) {
            ...
            hooks.injectHead.push((config, pageContext, root) => {
    ...
    if (typeof plugin.generateScripts === 'function') {
        if (hasCapabilityForHook(descriptor, 'generateScripts')) {
            ...
            hooks.injectHead.push((config, pageContext) => {
                if (!shouldExecute(pageContext))
                    return '';
                const result = safeCall('generateScripts', name, fn, config, options);
                return result?.headScriptsHtml || '';
            });
            hooks.injectBody.push((config, pageContext) => {
                if (!shouldExecute(pageContext))
                    return '';
                const result = safeCall('generateScripts', name, fn, config, options);
    ...
    if (typeof plugin.getAssets === 'function') {
        if (hasCapabilityForHook(descriptor, 'getAssets')) {
            ...
            hooks.assets.push(() => safeCall('getAssets', name, fn, options) || []);
        }
        else {
    ...
    if (typeof plugin.translations === 'function') {
        if (hasCapabilityForHook(descriptor, 'translations')) {
            ...
            hooks.translations.push((localeId) => safeCall('translations', name, fn, localeId, options) || {});
        }
        else {
